...
void static CoinracMiner(const CChainParams& chainparams)
{
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("CoinracMiner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("CoinracMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("CoinracMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("CoinracMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("CoinracMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateCoinracs(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&CoinracMiner, boost::cref(chainparams)));
    }
