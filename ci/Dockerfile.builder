...
# coinrac_hash
RUN git clone https://github.com/coinrac/coinrac_hash
RUN cd coinrac_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} coinrac
RUN useradd -u ${USER_ID} -g coinrac -s /bin/bash -m -d /coinrac coinrac
...
RUN mkdir /coinrac-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /coinrac-src && \
