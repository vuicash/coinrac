...
PACKAGE_VERSION=$($SRC_PATH/src/coinracd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/coinracd $DEB_BIN
cp $SRC_PATH/src/coinrac-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/coinrac-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/coinracd.1 $DEB_MAN
cp $SRC_DOC/man/coinrac-cli.1 $DEB_MAN
cp $SRC_DOC/man/coinrac-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/coinracd.bash-completion $DEB_CMP/coinracd
cp $SRC_PATH/contrib/coinrac-cli.bash-completion $DEB_CMP/coinrac-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/coinracd.1
gzip --best -n $DEB_MAN/coinrac-cli.1
gzip --best -n $DEB_MAN/coinrac-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/coinracd $DEB_BIN/coinrac-cli
