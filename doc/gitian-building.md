...
*Setup instructions for a Gitian build of Coinrac Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Coinrac
...
Clone the git repositories for Coinrac Core and Gitian.
...
in the Coinrac Core repository under 'Fetch and create inputs' to install sources which require
...
To build Coinrac Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Coinrac Core repository.
