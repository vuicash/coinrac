...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 COINRAC irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on coinraccentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in coinrac.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("coinrac.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in coinrac.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Coinrac features */
    { "coinrac",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "coinrac",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "coinrac",               "gobject",                &gobject,                true,  {} },
    { "coinrac",               "voteraw",                &voteraw,                true,  {} },
